...
/* returns the x ray spectral emissivity */
double xray_emissivity(double nu, int flag);
...
double integrate_over_nu(double zp, double zpp, double local_x_e, double lower_int_limit, int FLAG);
...
int init_heat()
{
  ...
  if (spectral_emissivity(0,1) < 0)
    return -6;
  if (xray_emissivity(NU_X_THRESH,1) < 0)
    return -7;
...
double xray_emissivity(double nu, int flag)
{
  static double nuk[xray_NPTS],lum[xray_NPTS];
  static gsl_interp_accel *acc;
  static gsl_spline *spline;
  double ans;
  int i;
  float curr_nu, curr_lum;
  FILE *F;

  if (flag == 1) {
    if (!(F=fopen(xray_spectra_FILENAME, "r"))){
      fprintf(stderr, "Unable to open the xray file at %s\nAborting\n", xray_spectra_FILENAME);
      fprintf(LOG, "Unable to open the xray file at %s\nAborting\n", xray_spectra_FILENAME);      
      return 0;
    }


    for (i=0;i<xray_NPTS;i++) {
      fscanf(F, "%e %e", &curr_nu, &curr_lum);
      nuk[i] = curr_nu;
      lum[i] = curr_lum;
    }
    fclose(F);
    ...
    spline  = gsl_spline_alloc (gsl_interp_cspline, xray_NPTS);
    gsl_spline_init(spline, nuk, lum, xray_NPTS);
    return 0;
  }
  if (flag == 2) {
    /* Free memory */
  ...
  else if (nu > nuk[xray_NPTS-1]){
    ans=lum[xray_NPTS-1];
  }
  else { /* Do spline */
    ans = gsl_spline_eval (spline, nu, acc);
  }
  ans = exp(ans);
  return ans;
}
...
void evolveInt(float zp, float curr_delNL0[], double freq_int_heat[], 
  ...
  for (zpp_ct = 0; zpp_ct < NUM_FILTER_STEPS_FOR_Ts; zpp_ct++){
    ...
    zpp_integrand = dfcoll * (1+curr_delNL0[zpp_ct]*dicke(zpp));
...
double integrand_in_nu_heat_integral(double nu, void * params){
  ...
  integration_params *p = (integration_params *) params;
  // HI
  species_sum = interp_fheat((nu - NUIONIZATION)/NU_over_EV, p->x_e)
               * hplank*(nu - NUIONIZATION) * f_H * (1-p->x_e) * HI_ion_crosssec(nu);

  // HeI
  species_sum += interp_fheat((nu - HeI_NUIONIZATION)/NU_over_EV, p-> x_e)
               * hplank*(nu - HeI_NUIONIZATION) * f_He * (1-p->x_e) * HeI_ion_crosssec(nu);

  // HeII
  species_sum += interp_fheat((nu - HeII_NUIONIZATION)/NU_over_EV,p->x_e)
               * hplank*(nu - HeII_NUIONIZATION) * f_He * p->x_e * HeII_ion_crosssec(nu);

  return species_sum * xray_emissivity(nu*((1+p->zpp)/(1+p->zp)),0)/nu;
}
...
double integrate_over_nu(double zp, double zpp, double local_x_e, double lower_int_limit, int FLAG){
	 ...
	 = gsl_integration_workspace_alloc (1000);
       ...
       if (DEBUG_ON){
	 printf("integrate over nu, parameters: %f, %f, %f, %e, %i, thread# %i\n", zp, zpp, local_x_e, lower_int_limit, FLAG, omp_get_thread_num());
       }
       /*
